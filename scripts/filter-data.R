...
cols_to_keep <- c("id", "title", "genres", "dateGlobal", "category",
                  "isFree", "amount", "baseAmount",
                  "overallAvgRating", "supportedOperatingSystems", "type")       # Liste des colonnes à conserver
...
if (length(cols_missing) > 0) {
  warning(paste("Col didn't found in the CSV :", paste(cols_missing, collapse = ", ")))
}
